<template>
  <div>
    <dialog id="deletUser" class="modal">
      ...
      <div class="holder">
        ...
        <div style="text-align: left; margin-right: auto; padding: 10px;">
          <p style="">This will delete the user: {{ this.for_user }}. <br><br>
             All API keys will be removed and active sessions will be terminated. <br> 
             Only do this if you are certain this account is no longer required.</p>
             <br>
             
          <p :class="this.successfull ? 'success':'normal'">{{ this.changeStatus  }}</p>
        ...
        <div style="text-align: center; margin-left: auto; margin-right: auto;">
          ...
          <button v-if="!this.successfull" class="glass-button" @click="deleteUser()">Delete User</button>
...
<script>
  ...
  export default {
    ...
    methods: {
      showModal: function (foruser) {
        this.for_user = foruser
        this.successfull = false
        console.log("showingDeleteUser")
        var e = document.getElementById('deletUser')
        e.showModal()
      },

      hideModal: function () {
        console.log("hidingDeleteUser")
        var e = document.getElementById('deletUser')
        e.close()
      },
      ...
      deleteUser: async function () {
        ...
        var response = await this.$refs.ApiConnect.send_post(`/user/profile/${this.for_user}/delete/`, payload, {}, false)

        if (response != undefined && "errors" in response && response['errors'].length > 0) {
          this.changeStatus = response['errors'][0]
        }

        if (response != undefined && "result" in response && "deleted_user" in response['result']) {
          this.changeStatus = `Successfully deleted ${this.for_user}`
          this.successfull = true
        } else {
          this.changeStatus = "Error deleting user"
        }
      }
